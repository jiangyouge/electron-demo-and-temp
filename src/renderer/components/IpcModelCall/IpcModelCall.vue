<template>
  <div class="ipc-centent">
    ...
    <div class="btn" @click=openNewWindow()>打开github新窗口</div>
    <div class="btn" @click=openNewPage()>打开newpage窗口</div>
    <div class="btn" @click=logRemoteMsg()>打印remote模块得到的主线程的信息</div>
    <div class="btn" v-if="callMsg">{{callMsg}}</div>
    <div class="btn" @click=callTwoDog()>呼叫二狗子</div>
    <div class="btn" @click=closeCall()>挂机</div>
...
export default {
  ...
  methods: {
    ...
    openNewPage () {
      ipcRenderer.sendSync('synchronous-openNewPage', 'willOpen')
    },
    logRemoteMsg () {
    ...
    callTwoDog () {
      remote.getGlobal('newPage2Window').webContents.send('webContents-call-Two-dog', {msg: '二狗子在的'})
    },
    closeCall () {
      this.callMsg = ''
    },
  ...
  mounted () {
    ...
    ipcRenderer.on('webContents-call-Three-kit', (e, msg = {msg: ''}) => {
      console.log('webContents-call-Three-kit - call == ', msg.msg)
